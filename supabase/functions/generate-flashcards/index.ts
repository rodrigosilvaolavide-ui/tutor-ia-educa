...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `Eres un generador de flash cards educativas para estudiantes de colegio en Latinoamérica. 
...
- Las preguntas deben evaluar comprensión, no solo memoria.`;
    ...
    const userPrompt = `Genera ${numCards} flash cards para el curso "${courseName}"${topic ? `, tema: "${topic}"` : ''}.`;
